...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # dLLM block-commit flag.  ONLY set by JointThreshold.run(); must not be
    # used for any other purpose.
    #   True  — denoising loop pass: skip KV-cache writes (flashinfer) AND
    #            hybrid S/Z state writes (LLaDAFastAttnBackend) so the pool
    #            stays at the last committed checkpoint for the entire loop.
    #   False — commit pass (default): both cache systems write normally.
    # Coupling both systems to one flag ensures they are never out of step.
    dllm_skip_kv_cache: bool = False
