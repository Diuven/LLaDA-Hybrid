...
def attn_backend_wrapper(runner: "ModelRunner", full_attn_backend: "AttentionBackend"):
    ...
    if cfg := runner.mambaish_config:
        ...
        elif runner.llada_fast_config is not None:
            from sglang.srt.layers.attention.llada_fast_backend import (
                LLaDAFastAttnBackend,
            )
            logger.info("Using LLaDAFast hybrid attention backend.")
            linear_attn_backend = LLaDAFastAttnBackend(runner)
        else:
            raise ValueError(
